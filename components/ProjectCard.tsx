...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelect }) => {
  ...
  return (
    <div 
      className="bg-white rounded-lg shadow-md overflow-hidden cursor-pointer transform hover:-translate-y-1 transition-transform duration-300 group flex flex-col relative border border-gray-200"
      onClick={() => onSelect(project)}
    >
      <div className="absolute top-3 left-3 z-10 bg-gray-100 text-gray-600 rounded-full h-8 w-8 flex items-center justify-center font-bold text-sm border border-gray-200">
        {project.rank}
      </div>
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h3 className="font-bold text-lg truncate text-gray-800" title={project.teamName}>{project.teamName}</h3>
        <p className="text-sm text-gray-500 mt-1 flex-grow h-10 overflow-hidden" title={project.highlights}>
            {project.highlights}
        </p>
