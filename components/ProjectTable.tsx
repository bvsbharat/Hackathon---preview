...
const EditableCell: React.FC<{
  ...
  if (isEditing) {
    return (
      <input
        ...
        className="bg-gray-900 text-white w-full border-0 rounded-sm outline-none ring-2 ring-cyan-500 px-2 py-1"
  ...
  return (
    <div
      ...
      className={`cursor-text w-full h-full p-1 -m-1 rounded-sm hover:bg-gray-700/50 transition-colors ${className}`}
      title="Click to edit"
    >
      {String(value)}
    </div>
          ...
          <table className="min-w-full divide-y divide-gray-700">
            <thead className="bg-gray-700/50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Links</th>
              </tr>
            </thead>
              ...
              {projects.map((project, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold">
                     <EditableCell value={project.total} projectIndex={index} columnKey="total" onUpdate={handleCellUpdate} className="text-cyan-400" />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-4">
                      {project.githubLink && project.githubLink !== 'N/A' && (
                         <a href={project.githubLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-gray-400 hover:text-white transition-colors" aria-label="GitHub link">
                           <GitHubIcon className="w-5 h-5" />
                         </a>
                      )}
                      {project.youtubeLink && (
                         <a href={project.youtubeLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-gray-400 hover:text-white transition-colors" aria-label="Demo link">
